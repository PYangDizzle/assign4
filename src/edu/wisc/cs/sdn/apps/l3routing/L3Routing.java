       ...
       ILinkDiscoveryListener, IDeviceListener
{
	...
	private void connectSwitches() {
		...
		for( Link link : links ) {
			...
			IOFSwitch srcSw = floodlightProv.getSwitch( link.getSrc() );
			IOFSwitch dstSw = floodlightProv.getSwitch( link.getDst() );
	...
	private void updateMap( boolean switchChanged ) {
		if( switchChanged ) {
			// clears shortestRoutesMap then rebuild the routes between switches
			connectSwitches();
			buildRoutes();
		}
		// install rules
